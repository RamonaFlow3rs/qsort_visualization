...
final class VisualSortArray extends AbstractSortArray {
	...
	VisualSortArray(int size, int scale, double speed, boolean isChecked) {
	...
	VisualSortArray(int scale, double speed, int[] values){
		...
		// Initialize graphics
		this.scale = scale;
		canvas = new BufferedCanvas((size) * scale, (max) * scale);
		graphics = canvas.getBufferGraphics();
		redraw(0, size);
	...
	private void redraw(int start, int end) {
		...
		graphics.fillRect(start * scale, 0,  (end - start ) * scale , (max)* scale);
		for (int i = start; i < end; i++) {
			graphics.setColor(STATE_COLORS[state[i]]);
			if (scale == 1)
				graphics.drawLine(i, max, i, max-values[i]);
			else{
				graphics.fillRect(i*scale, max * scale - (values[i] ) * scale, scale, (values[i]) * scale);
			}  // scale > 1
